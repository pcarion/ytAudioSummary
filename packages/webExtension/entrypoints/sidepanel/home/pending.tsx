...
export function PendingSubmission({
  ...
  const { submissionId, title, url, date, creditsCost } = submission;
      ...
      <div className="flex items-center justify-between gap-4">
        <div
          className={cn(
            ...
            "w-14 h-14 text-xs font-bold whitespace-pre",
            "shadow border border-blue-700 select-none"
          )}
        >
          <span className="leading-tight text-lg">{creditsCost}</span>
          <span className="leading-tight">{creditsCost > 1 ? 'credits' : 'credit'}</span>
        </div>
