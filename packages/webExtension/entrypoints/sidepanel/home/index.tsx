...
export function Home({ onNavigateToFeed, onNavigateToSettings }: HomeProps) {
  ...
  const loadUserData = async () => {
    try {
      ...
      const newPendingSubmissions = data.lastSubmissions.filter(
        (s) => s.approvalStatus === "notConfirmed"
      );
        ...
        if (response.success) {
          ...
          if (result.success) {
            addPendingSubmission({
              ...
              date: "",
              creditsCost: result.credits.creditsCost,
            });
          }
          console.log("Result:");
          console.log(result);
      ...
      <div className="h-screen w-full bg-white dark:bg-gray-900 p-4 flex flex-col relative">
        ...
        {userData && (
          <div className="flex gap-3 mb-4">
            <CreditsPanel
              availableCredits={userData.credits.availableCredits}
              lastUpdate={userData.credits.lastUpdate}
