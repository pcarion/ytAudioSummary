...
import type {
  ...
  GetPageYoutubeVideoDetails,
  GetPageContentTranscriptInformation,
...
export default defineContentScript({
  ...
  async main(ctx: ContentScriptContext) {
    console.log("content script loaded:", ctx.options?.matches);
      ...
      try {
        ...
        if (isYoutubeVideoPage) {
          ...
          for (const script of scriptTags) {
            ...
            if (
              scriptVariables.ytInitialPlayerResponse &&
              scriptVariables.ytInitialData
            ) {
              break;
            }
            if (script.innerHTML) {
              if (!scriptVariables.ytInitialPlayerResponse) {
                scriptVariables.ytInitialPlayerResponse = extractJsonFromHtml(
                  script.innerHTML,
                  "ytInitialPlayerResponse"
                );
              }
              if (!scriptVariables.ytInitialData) {
                scriptVariables.ytInitialData = extractJsonFromHtml(
                  script.innerHTML,
                  "ytInitialData"
                );
              }
          ...
          if (scriptVariables.ytInitialPlayerResponse) {
            ...
            const youtubeVideoInformation = await parse_ytInitialPlayerResponse(
              scriptVariables.ytInitialPlayerResponse
            );
            ...
            if (youtubeVideoInformation.isValid) {
              ...
              transcriptInformation.transcript =
                youtubeVideoInformation.transcript;
              hasTranscript = transcriptInformation.transcript.length > 20;

              if (youtubeVideoInformation.videoDetails) {
                youtubeDetails.videoId =
                  youtubeVideoInformation.videoDetails.videoId;
                youtubeDetails.title =
                  youtubeVideoInformation.videoDetails.title;
                youtubeDetails.author =
                  youtubeVideoInformation.videoDetails.author;
                youtubeDetails.shortDescription =
                  youtubeVideoInformation.videoDetails.shortDescription;
                youtubeDetails.isLiveContent =
                  youtubeVideoInformation.videoDetails.isLiveContent;
                youtubeDetails.lengthSeconds =
                  youtubeVideoInformation.videoDetails.lengthSeconds;
                youtubeDetails.channelId =
                  youtubeVideoInformation.videoDetails.channelId;
                youtubeDetails.thumbnails =
                  youtubeVideoInformation.videoDetails.thumbnail.thumbnails;
              }
            }
          }
          ...
          if (scriptVariables.ytInitialData && !hasTranscript) {
            ...
            console.log(scriptVariables.ytInitialData);
            const ytInitialDataInformation = await parse_ytInitialData(
              scriptVariables.ytInitialData
            );
            ...
            if (ytInitialDataInformation.hasTranscript) {
              transcriptInformation.language =
                ytInitialDataInformation.language;
              transcriptInformation.transcript =
                ytInitialDataInformation.transcript;
              hasTranscript = transcriptInformation.transcript.length > 20;
            }
          }
        } else {
